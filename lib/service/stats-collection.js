...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'dogechia-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('dogechia').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('dogechia')[service]);
